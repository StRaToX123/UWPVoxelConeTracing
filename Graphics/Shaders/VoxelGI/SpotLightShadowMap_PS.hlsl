...
float main(VertexShaderOutputSpotLightShadowMap input) : SV_Target
{
	...
	projectTexCoord.x = input.position_light_view_proj_space.x / input.position_light_view_proj_space.w / 2.0f + 0.5f;
	projectTexCoord.y = -input.position_light_view_proj_space.y / input.position_light_view_proj_space.w / 2.0f + 0.5f;
	...
	if ((saturate(projectTexCoord.x) == projectTexCoord.x) && (saturate(projectTexCoord.y) == projectTexCoord.y))
	{
		...
		vertexLightDepthValue = input.position_light_view_proj_space.z / input.position_light_view_proj_space.w;
		// Compare the depth of the shadow map value and the depth of the light to determine whether to shadow or to light this pixel.
		// If the light is in front of the object then light the pixel, if not then shadow this pixel since an object (occluder) is casting a shadow on it.
		if (vertexLightDepthValue < depthValue)
		{
		    // Calculate the amount of light on this pixel.
			lightIntensity = saturate(dot(input.normal_world_space, input.light_pos_world_space));
