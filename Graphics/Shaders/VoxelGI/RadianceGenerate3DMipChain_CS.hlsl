...
ConstantBuffer<ShaderStructureGPUGenerate3DMipChainData> generate_3d_mip_chain_data : register(b5);
Texture3D<min16float4> radiance_texture_3D_SRVs[] : register(t4);
RWTexture3D<min16float4> radiance_texture_3D_UAVs[] : register(u4);
...
void main(uint3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	{
		Texture3D<min16float4> input = radiance_texture_3D_SRVs[generate_3d_mip_chain_data.input_mip_index];
		RWTexture3D<min16float4> output = radiance_texture_3D_UAVs[generate_3d_mip_chain_data.output_mip_index];

		min16float4 color = input.SampleLevel(linear_sampler, ((float3) dispatchThreadID + 0.5f) * (float3) generate_3d_mip_chain_data.output_resolution_rcp, 0);
