...
Texture3D<float4> radiance_texture_3D_SRVs[] : register(t5); // <-------- Different registers, in order to read and write from the helper texture 3D
RWTexture3D<float4> radiance_texture_3D_UAVs[] : register(u6); // <-------- Different registers, in order to read and write from the helper texture 3D
