...
VoxelDebugDrawPixelShaderInput main(VoxelDebugDrawVertexShaderInput input)
{
	...
	output.position += float4(voxel_grid_data.top_left_point_world_space_x + voxel_grid_data.voxel_half_extent, voxel_grid_data.top_left_point_world_space_y - voxel_grid_data.voxel_half_extent, voxel_grid_data.top_left_point_world_space_z + voxel_grid_data.voxel_half_extent, 0.0f);
	// Move the voxel debug cube to the apropriate position in the grid based on its index
	...
	output.color = radiance_texture_3D_UAV[uint3(voxel_debug_data_required_for_frame_draw[input.instance_id].index_x, voxel_debug_data_required_for_frame_draw[input.instance_id].index_y, voxel_debug_data_required_for_frame_draw[input.instance_id].index_z)];
