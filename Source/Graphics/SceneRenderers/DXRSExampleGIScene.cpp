...
void DXRSExampleGIScene::Initialize(Windows::UI::Core::CoreWindow^ coreWindow)
{
	//mKeyboard = std::make_unique<DirectX::Keyboard>();
	//mMouse = std::make_unique<DirectX::Mouse>();
	//mMouse->SetWindow(coreWindow);
	mSandboxFramework->SetWindow(coreWindow);
	mSandboxFramework->CreateResources();
	mSandboxFramework->CreateFullscreenQuadBuffers();

	descriptor_heap_manager.Initialize(mSandboxFramework->GetD3DDevice());

	DXRSModel* model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\room.fbx"), XMMatrixRotationX(-XM_PIDIV2), XMFLOAT4(0.7, 0.7, 0.7, 0.0));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\dragon.fbx"), XMMatrixTranslation(1.5f, 0.0f, -7.0f), XMFLOAT4(0.044f, 0.627f, 0, 0.0));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\bunny.fbx"), XMMatrixIdentity() * XMMatrixRotationY(-0.3752457f) * XMMatrixTranslation(21.0f, 13.9f, -19.0f), XMFLOAT4(0.8f, 0.71f, 0, 0.0));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\torus.fbx"), XMMatrixIdentity() * XMMatrixRotationX(-XM_PIDIV2) * XMMatrixRotationX(1.099557f) * XMMatrixTranslation(21.0f, 4.0f, -9.6f), XMFLOAT4(0.329f, 0.26f, 0.8f, 0.8f));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\sphere_big.fbx"), XMMatrixIdentity() * XMMatrixTranslation(-17.25f, -1.15f, -24.15f), XMFLOAT4(0.692f, 0.215f, 0.0f, 0.6f));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\sphere_medium.fbx"), XMMatrixIdentity() * XMMatrixTranslation(-21.0f, -0.95f, -13.20f), XMFLOAT4(0.005, 0.8, 0.426, 0.7f));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\sphere_small.fbx"), XMMatrixIdentity() * XMMatrixTranslation(-11.25f, -0.45f, -16.20f), XMFLOAT4(0.01, 0.0, 0.8, 0.75f));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\block.fbx"), XMMatrixIdentity() * XMMatrixRotationX(-XM_PIDIV2) * XMMatrixTranslation(3.0f, 8.0f, -30.0f), XMFLOAT4(0.9, 0.15, 1.0, 0.0));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\cube.fbx"), XMMatrixIdentity() * XMMatrixRotationX(-XM_PIDIV2) * XMMatrixRotationY(-0.907571f) * XMMatrixTranslation(21.0f, 5.0f, -19.0f) , XMFLOAT4(0.1, 0.75, 0.8, 0.0));
	mRenderableObjects.emplace_back(model);

	model = new DXRSModel(mSandboxFramework, &descriptor_heap_manager, mSandboxFramework->GetFilePath("Assets\\Models\\sphere_medium.fbx"), XMMatrixIdentity() * XMMatrixTranslation(RandomFloat(-35.0f, 35.0f), RandomFloat(5.0f, 30.0f), RandomFloat(-35.0f, 35.0f)), XMFLOAT4(RandomFloat(0.0f, 1.0f), RandomFloat(0.0f, 1.0f), RandomFloat(0.0f, 1.0f), 0.8), true, RandomFloat(-1.0f, 1.0f), RandomFloat(1.0f, 5.0f));
	mRenderableObjects.emplace_back(model);
	for (int i = 0; i < NUM_DYNAMIC_OBJECTS - 1; i++)
	{
		DXRSModel* modelCopy = new DXRSModel(mSandboxFramework, 
		...
		*modelCopy = *model;
		mRenderableObjects.emplace_back(modelCopy);
	}
...
#pragma endregion
	...
	mGIUpsampleAndBlurBuffer = new DX12Buffer(mSandboxFramework->GetD3DDevice(), &descriptor_heap_manager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"GI Upsample & Blur CB");
	mDXRBlurBuffer = new DX12Buffer(mSandboxFramework->GetD3DDevice(), &descriptor_heap_manager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"DXR Blur CB");
	...
	//InitReflectiveShadowMapping(device, &descriptor_heap_manager);
	//InitLightPropagationVolume(device, &descriptor_heap_manager);
	InitVoxelConeTracing(device, &descriptor_heap_manager);
	//InitReflectionsDXR(device, &descriptor_heap_manager);
...
void DXRSExampleGIScene::Run(DXRSTimer& mTimer, Camera& camera)
{
	mTimer.Run([&]()
	{
			directional_light_animation_angle += static_cast<float>(mTimer.GetElapsedSeconds()) * 45.0f;
			...
			{
				mDirectionalLightDir[0] = 0.6f * cos(directional_light_animation_angle * (3.14f / 180.0f));
			}
...
void DXRSExampleGIScene::Render(DXRSTimer& mTimer)
{
	...
	//process compute command list - async during the frame (GI)
	{
		if (mTimer.GetFrameCount() > 1) 
		{
			...
			RenderVoxelConeTracing(device, commandListCompute, gpuDescriptorHeap, COMPUTE_QUEUE);
	...
	//process graphics command list 2 - middle of the frame (Shadows, GI) 
	{
		if (mTimer.GetFrameCount() > 1) 
		{
			...
			//RenderReflectiveShadowMapping(device, commandListGraphics2, gpuDescriptorHeap, GRAPHICS_QUEUE); //only rsm textures generation which cant go to compute
			//RenderLightPropagationVolume(device, commandListGraphics2, gpuDescriptorHeap);
			RenderVoxelConeTracing(device, commandListGraphics2, gpuDescriptorHeap, GRAPHICS_QUEUE);//only voxelization there which cant go to compute
...
void DXRSExampleGIScene::Update(DXRSTimer const& timer, Camera& camera)
{
...
void DXRSExampleGIScene::UpdateBuffers(DXRSTimer const& timer)
{
	...
	gbufferPassData.InvViewProjection = XMMatrixTranspose(XMMatrixInverse(nullptr, XMMatrixMultiply(mCameraView, mCameraProjection)));
	...
	XMMATRIX translationMatrix = DirectX::XMMatrixTranslation(-shadowCenter.x, -shadowCenter.y, -shadowCenter.z);
	XMVECTOR rotationQuaternion = XMQuaternionRotationMatrix(DirectX::XMMatrixTranspose(DirectX::XMMatrixLookToLH(eyePosition, direction, upDirection)));
	...
	XMMATRIX projectionMatrix = XMMatrixOrthographicLH(256.0f, 256.0f, -256.0f, 256.0f);
	mLightViewProjection = XMMatrixMultiply(viewMatrix, projectionMatrix);
	mLightView = viewMatrix;
	mLightProj = projectionMatrix;

	ShadowMappingCBData shadowPassData = {};
	shadowPassData.LightViewProj = XMMatrixTranspose(mLightViewProjection);
	...
	rsmPassData.ShadowViewProjection = XMMatrixTranspose(mLightViewProjection);
	...
	/*DXRBuffer dxrData = {};
	dxrData.ViewMatrix = XMMatrixTranspose(mCameraView);
	dxrData.ProjectionMatrix = XMMatrixTranspose(mCameraProjection);
	dxrData.InvViewMatrix = XMMatrixTranspose(XMMatrixInverse(nullptr, mCameraView));
	dxrData.InvProjectionMatrix = XMMatrixTranspose(XMMatrixInverse(nullptr, mCameraProjection));
	dxrData.ShadowViewProjection = XMMatrixTranspose(mLightViewProjection);
...
void DXRSExampleGIScene::UpdateCamera(DXRSTimer const& timer, Camera& camera)
{
	//mCamera->Update(timer, mMouse, mKeyboard);
	//mCameraView = camera.ViewMatrix();
	//mCameraProjection = camera.ProjectionMatrix();
	//mCameraEye = camera.Position();
...
void DXRSExampleGIScene::OnWindowSizeChanged(Camera& camera)
{
	if (!mSandboxFramework->WindowSizeChanged())
		return;
	...
	float aspectRatio = mSandboxFramework->mAppWindow->Bounds.Width / mSandboxFramework->mAppWindow->Bounds.Height;
...
void DXRSExampleGIScene::InitGbuffer(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	...
	LoadShaderByteCode(mSandboxFramework->GetFilePath("GBuffer_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
	LoadShaderByteCode(mSandboxFramework->GetFilePath("GBuffer_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
	...
	mGbufferCB = new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"GBuffer CB");
}
void DXRSExampleGIScene::RenderGbuffer(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap)
{
	CD3DX12_VIEWPORT viewport = CD3DX12_VIEWPORT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
	CD3DX12_RECT rect = CD3DX12_RECT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
...
void DXRSExampleGIScene::InitShadowMapping(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
// create resources for shadow mapping
{
	...
	char* pVertexShaderByteCode;
	int vertexShaderByteCodeLength;
	LoadShaderByteCode(mSandboxFramework->GetFilePath("ShadowMapping_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
	...
	mShadowMappingCB = new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"Shadow Mapping CB");
}
void DXRSExampleGIScene::RenderShadowMapping(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap)
{
	CD3DX12_VIEWPORT viewport = CD3DX12_VIEWPORT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
	CD3DX12_RECT rect = CD3DX12_RECT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
...
void DXRSExampleGIScene::InitReflectiveShadowMapping(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	//generation
	{
		D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\ShadowMapping.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_1", compileFlags, 0, &vertexShader, nullptr));

		compileFlags |= D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES;

		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\ShadowMapping.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSRSM", "ps_5_1", compileFlags, 0, &pixelShader, &errorBlob));
	...
	//calculation
	{
		...
		// pixel version
		{
			...
			ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\ReflectiveShadowMappingPS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, &errorBlob));
			if (errorBlob)
			{
			...
			ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\ReflectiveShadowMappingPS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, &errorBlob));
			if (errorBlob)
			{
				OutputDebugStringA((char*)errorBlob->GetBufferPointer());
				errorBlob->Release();
		...
		// compute version
		{
			...
			ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\ReflectiveShadowMappingCS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CSMain", "cs_5_0", compileFlags, 0, &computeShader, &errorBlob));
			if (errorBlob)
			{
				OutputDebugStringA((char*)errorBlob->GetBufferPointer());
				errorBlob->Release();
		...
		mRSMCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"RSM Pass CB");

		cbDesc.mElementSize = sizeof(RSMCBDataRandomValues);
		mRSMCB2 = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"RSM Pass CB 2");
	...
	//blur
	{
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\UpsampleBlurCS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CSMain", "cs_5_0", compileFlags, 0, &computeShader, &errorBlob));
		if (errorBlob)
		{
			OutputDebugStringA((char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
	...
	//downsampling for LPV - PS
	{
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\RSMDownsamplePS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_1", compileFlags, 0, &vertexShader, &errorBlob));

		compileFlags |= D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES;

		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\RSMDownsamplePS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_1", compileFlags, 0, &pixelShader, &errorBlob));
		...
		mRSMDownsampleCB = new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"RSM Downsample CB");
	}
	...
	//downsampling for LPV - CS
	{
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\RSMDownsampleCS.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CSMain", "cs_5_0", compileFlags, 0, &computeShader, &errorBlob));
		if (errorBlob)
		{
			OutputDebugStringA((char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
...
void DXRSExampleGIScene::RenderReflectiveShadowMapping(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue)
{
	if (!mUseRSM && !mUseLPV)
		return;
		...
		if (mRSMDownsampleForLPV) {
			if (aQueue == GRAPHICS_QUEUE) 
			{
				//PIXBeginEvent(commandList, 0, "RSM downsample PS for LPV");
				{
					...
					DX12DescriptorHandle cbvHandleRSM = gpuDescriptorHeap->GetHandleBlock(1);
					gpuDescriptorHeap->AddToHandle(device, cbvHandleRSM, mRSMDownsampleCB->GetCBV());

					DX12DescriptorHandle srvHandleRSM = gpuDescriptorHeap->GetHandleBlock(3);
			...
			else if (aQueue == GRAPHICS_QUEUE) 
			{
				//PIXBeginEvent(commandList, 0, "RSM downsample CS for LPV");
				{
					...
					DX12DescriptorHandle uavHandleRSM = gpuDescriptorHeap->GetHandleBlock(3);
	...
	if (mUseRSM) {
		...
		if (aQueue == GRAPHICS_QUEUE) 
		{
			//PIXBeginEvent(commandList, 0, "RSM main calculation PS");
			{
				...
				DX12DescriptorHandle cbvHandleRSM = gpuDescriptorHeap->GetHandleBlock(2);
				...
				DX12DescriptorHandle srvHandleRSM = gpuDescriptorHeap->GetHandleBlock(5);
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs[0]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs[1]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs[2]->GetSRV());
		...
		else if (aQueue == COMPUTE_QUEUE)
		{
			//PIXBeginEvent(commandList, 0, "RSM main calculation CS");
			{
				...
				DX12DescriptorHandle srvHandleRSM = gpuDescriptorHeap->GetHandleBlock(5);
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs_CopiesForAsync[0]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs_CopiesForAsync[1]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mRSMBuffersRTs_CopiesForAsync[2]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mGbufferRTs[2]->GetSRV());
				gpuDescriptorHeap->AddToHandle(device, srvHandleRSM, mGbufferRTs[1]->GetSRV());

				DX12DescriptorHandle uavHandleRSM = gpuDescriptorHeap->GetHandleBlock(1);
		...
		if (aQueue == COMPUTE_QUEUE) 
		{
			...
			//PIXBeginEvent(commandList, 0, "RSM upsample & blur CS");
			{
				...
				DX12DescriptorHandle srvHandleBlurRSM = gpuDescriptorHeap->GetHandleBlock(1);
				gpuDescriptorHeap->AddToHandle(device, srvHandleBlurRSM, mRSMRT->GetSRV());

				DX12DescriptorHandle uavHandleBlurRSM = gpuDescriptorHeap->GetHandleBlock(1);
				gpuDescriptorHeap->AddToHandle(device, uavHandleBlurRSM, mRSMUpsampleAndBlurRT->GetUAV());

				DX12DescriptorHandle cbvHandleBlurRSM = gpuDescriptorHeap->GetHandleBlock(1);
...
void DXRSExampleGIScene::InitLightPropagationVolume(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	// injection
	{
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVInjection.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_1", compileFlags, 0, &vertexShader, &errorBlob));
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVInjection.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "GSMain", "gs_5_1", compileFlags, 0, &geometryShader, &errorBlob));
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVInjection.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_1", compileFlags, 0, &pixelShader, &errorBlob));
		...
		mLPVCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"LPV Injection Pass CB");
	...
	// propagation
	{
		...
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVPropagation.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_1", compileFlags, 0, &vertexShader, &errorBlob));
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVPropagation.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "GSMain", "gs_5_1", compileFlags, 0, &geometryShader, &errorBlob));
		ThrowIfFailed(D3DCompileFromFile(mSandboxFramework->GetFilePath(L"Source\\Graphics\\Shaders\\LPVPropagation.hlsl").c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_1", compileFlags, 0, &pixelShader, &errorBlob));
...
void DXRSExampleGIScene::RenderLightPropagationVolume(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue)
{
	CD3DX12_VIEWPORT viewport = CD3DX12_VIEWPORT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
	CD3DX12_RECT rect = CD3DX12_RECT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
	...
	if (mUseLPV) {
		//PIXBeginEvent(commandList, 0, "LPV Injection");
		{
			...
			DX12DescriptorHandle cbvHandleLPVInjection = gpuDescriptorHeap->GetHandleBlock(2);
			...
			DX12DescriptorHandle srvHandleLPVInjection = gpuDescriptorHeap->GetHandleBlock(3);
...
void DXRSExampleGIScene::InitVoxelConeTracing(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	// voxelization
	{
		...
		char* pPixelShaderByteCode;
		int pixelShaderByteCodeLength;
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelization_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelization_GS.cso"), pGeometryShadeByteCode, geometryShaderByteCodeLength);
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelization_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
		...
		cbDesc.mState = D3D12_RESOURCE_STATE_GENERIC_READ;
		cbDesc.mDescriptorType = DX12Buffer::DescriptorType::CBV;
		mVCTVoxelizationCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT Voxelization Pass CB");
	...
	//debug 
	{
		...
		char* pPixelShaderByteCode;
		int pixelShaderByteCodeLength;
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelizationDebug_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelizationDebug_GS.cso"), pGeometryShadeByteCode, geometryShaderByteCodeLength);
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingVoxelizationDebug_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
	...
	// aniso mipmapping prepare
	{
		...
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingAnisoMipmapPrepare_CS.cso"), pComputeShaderByteCode, computeShaderByteCodeLength);
		...
		mVCTAnisoMipmappingCB = new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping CB");
	}
	...
	// aniso mipmapping main
	{
		...
		char* pComputeShaderByteCode;
		int computeShaderByteCodeLength;
		LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracingAnisoMipmapMain_CS.cso"), pComputeShaderByteCode, computeShaderByteCodeLength);
		...
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 0 CB"));
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 1 CB"));
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 2 CB"));
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 3 CB"));
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 4 CB"));
		mVCTAnisoMipmappingMainCB.push_back(new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT aniso mip mapping main mip 5 CB"));
	...
	// main 
	{
		...
		mVCTMainCB = new DX12Buffer(mSandboxFramework->GetD3DDevice(), descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"VCT main CB");
		...
		// PS
		{
			...
			char* pPixelShaderByteCode;
			int pixelShaderByteCodeLength;
			LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracing_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
			LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracing_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
		...
		// CS
		{
			...
			char* pComputeShaderByteCode;
			int computeShaderByteCodeLength;
			LoadShaderByteCode(mSandboxFramework->GetFilePath("VoxelConeTracing_CS.cso"), pComputeShaderByteCode, computeShaderByteCodeLength);
	...
	// upsample and blur
	{
		...
		char* pComputeShaderByteCode;
		int computeShaderByteCodeLength;
		LoadShaderByteCode(mSandboxFramework->GetFilePath("UpsampleBlur_CS.cso"), pComputeShaderByteCode, computeShaderByteCodeLength);
...
void DXRSExampleGIScene::RenderVoxelConeTracing(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue)
{
	if (!mUseVCT)
	{
		return;
...
void DXRSExampleGIScene::InitLighting(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	//RTs
	...
	char* pPixelShaderByteCode;
	int pixelShaderByteCodeLength;
	LoadShaderByteCode(mSandboxFramework->GetFilePath("Lighting_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
	LoadShaderByteCode(mSandboxFramework->GetFilePath("Lighting_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
	...
	mLightingCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"Lighting Pass CB");

	cbDesc.mElementSize = sizeof(LightsInfoCBData);
	mLightsInfoCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"Lights Info CB");

	cbDesc.mElementSize = sizeof(IlluminationFlagsCBData);
	mIlluminationFlagsCB = new DX12Buffer(device, descriptorManager, mSandboxFramework->GetCommandListGraphics(), cbDesc, L"Illumination Flags CB");
}
void DXRSExampleGIScene::RenderLighting(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap)
{
	CD3DX12_VIEWPORT viewport = CD3DX12_VIEWPORT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
	CD3DX12_RECT rect = CD3DX12_RECT(0.0f, 0.0f, mSandboxFramework->GetOutputSize().right, mSandboxFramework->GetOutputSize().bottom);
...
void DXRSExampleGIScene::InitComposite(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager)
{
	//create root signature
	D3D12_ROOT_SIGNATURE_FLAGS rootSignatureFlags =
		D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT |
	...
	char* pPixelShaderByteCode;
	int pixelShaderByteCodeLength;
	LoadShaderByteCode(mSandboxFramework->GetFilePath("Composite_VS.cso"), pVertexShaderByteCode, vertexShaderByteCodeLength);
	LoadShaderByteCode(mSandboxFramework->GetFilePath("Composite_PS.cso"), pPixelShaderByteCode, pixelShaderByteCodeLength);
...
void DXRSExampleGIScene::RenderComposite(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap)
{
