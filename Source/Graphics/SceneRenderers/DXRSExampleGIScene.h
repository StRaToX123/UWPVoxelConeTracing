...
public:
	...
	void Run(DXRSTimer& mTimer, Camera& camera);
...
private:
	...
	void InitReflectiveShadowMapping(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager);
	void InitLightPropagationVolume(ID3D12Device* device, DX12DescriptorHeapManager* descriptorManager);
	...
	void Render(DXRSTimer& mTimer);

	void RenderGbuffer(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap);
	void RenderShadowMapping(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap);
	void RenderReflectiveShadowMapping(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue = GRAPHICS_QUEUE);
	void RenderLightPropagationVolume(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue = GRAPHICS_QUEUE);
	void RenderVoxelConeTracing(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12DescriptorHeapGPU* gpuDescriptorHeap, RenderQueue aQueue = GRAPHICS_QUEUE);
	...
	DX12DescriptorHeapGPU* mLPVPropagationBundle1UsedGPUHeap;
	DX12DescriptorHeapGPU* mLPVPropagationBundle2UsedGPUHeap;
	...
	{
		XMFLOAT4 inverse_light_dir_world_space; // Vector pointing towards the light
		XMFLOAT4 LightColor;
	...
	//float mDirectionalLightDir[4]{ 0.0f, -1.0f, 0.0f, 1.0 };
	float mDirectionalLightIntensity = 3.0f;
	bool mDynamicDirectionalLight = true;
	...
	DX12Buffer* mTLASBuffer; // top level acceleration structure of the scene
	DX12Buffer* mTLASScratchBuffer;
	DX12Buffer* mTLASInstanceDescriptionBuffer;
	...
	DX12Buffer*	mDXRBuffer; //cbuffer for DXR reflections pass
